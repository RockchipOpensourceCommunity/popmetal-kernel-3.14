...
static int rk32_lvds_disable(void)
{
	...
	ret = regmap_write(lvds->grf, RK3288_GRF_SOC_CON7, 0xffff8000);
	if (ret != 0) {
...
static int rk32_lvds_en(void)
{
	...
	ret = regmap_write(lvds->grf, RK3288_GRF_SOC_CON6, val);
	if (ret != 0) {
		dev_err(lvds->dev, "Could not write to GRF: %d\n", ret);
		return ret;
	}
	...
	ret = regmap_write(lvds->grf, RK3288_GRF_SOC_CON7, val);
	if (ret != 0) {
		dev_err(lvds->dev, "Could not write to GRF: %d\n", ret);
		return ret;
	}
	...
	if (screen->type == SCREEN_RGB) {
		...
		ret = regmap_write(lvds->grf, RK3288_GRF_GPIO1D_IOMUX, val);
		if (ret != 0) {
			dev_err(lvds->dev, "Could not write to GRF: %d\n", ret);
			return ret;
		}
...
static int rk32_lvds_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(lvds->grf)) {
		dev_err(lvds->dev,"needs rockchip,grf property\n");
