...
static int rk3288_hdmi_drv_init(struct hdmi *hdmi_drv)
{
	...
	/* lcdc source select */
	ret = regmap_write(hdmi_dev->grf, RK3288_GRF_SOC_CON6, HDMI_SEL_LCDC(hdmi_dev->lcdc_id));
	if (ret != 0) {
		dev_err(hdmi_dev->dev, "Could not write to GRF: %d\n", ret);
		return ret;
	}
...
static int rk3288_hdmi_probe(struct platform_device *pdev)
{
	...
	hdmi_dev->grf = syscon_regmap_lookup_by_phandle(np, "rockchip,grf");
	if (IS_ERR(hdmi_dev->grf)) {
		dev_err(hdmi_dev->dev,
				"needs rockchip,grf property\n");
		return PTR_ERR(hdmi_dev->grf);
	}
	...
	if (ret = rk3288_hdmi_drv_init(dev_drv))
		goto err0;
...
late_initcall(rk3288_hdmi_init);
